...
def identify_cvd_sub_class() -> None:
    ...
    result_dir = 'D:\\Dataset\\standardized_data\\ECG\\mimic'
    result_file_name = '03_ecg_cvd_sub_class.csv'
    file_name = os.path.join(result_dir,file_name)
    df = pd.read_csv(file_name)
...
def get_cvd_from_ed_event() -> None:
    """ 从 ed event 数据集中, 筛选所有 CVD 患者 """
    # 0. prepare
    base_dir = 'D:\\Dataset\\original_data\\ECG'
    dataset_name= '20240801_mimics_iv'
    sub_dataset = 'mimicel-mimic-iv-event-log-for-emergency-department-2.1.0'
    file_name = 'data\\mimicel.csv'
    # 1. 读取 ed event 数据列表, 按 icd_code 检索缺血性心肌病
    file_path = os.path.join(base_dir,dataset_name,sub_dataset,file_name)
    ed_event_df = pd.read_csv(file_path, encoding='GB18030')
    result_dir = 'D:\\Dataset\\standardized_data\\ECG\\mimic'
    result_file_name = '05_ed_event_cvd.csv'
    cvd_df = {}
    save_keys = ['stay_id','subject_id','hadm_id','timestamps','activity','gender','race',
                 'arrival_transport','disposition','seq_num','icd_code','icd_version','icd_title']

    for key in save_keys:
        cvd_df[key] = []
    # 2. 根据 icd code 筛选 CVD 患者
    all_diags = ed_event_df['icd_code'].values
    for idx, diag in enumerate(all_diags):
        ## icd version = 9
        if ed_event_df['icd_version'][idx] == 9:
            if diag.isnumeric():
                class_id = int(str(diag)[0:3])
                if class_id>=410 and class_id<=414:
                    for key in save_keys:
                        cvd_df[key].append(ed_event_df[key][idx])
        ## icd version = 10
        elif ed_event_df['icd_version'][idx] == 10:
            if 'I' in diag[0]:
                for key in save_keys:
                    cvd_df[key].append(ed_event_df[key][idx])
...
def identify_ed_icm_from_cvd() -> None:
    """ 从 ed event 数据集中, 筛选所有 CVD 患者 """
    # 0. prepare
    base_dir = 'D:\\Dataset\\original_data\\ECG'
    dataset_name= '20240801_mimics_iv'
    ...
    # 1. 读取 ed event 数据列表, 按 icd_code 检索缺血性心肌病
    file_path = os.path.join(base_dir,dataset_name,sub_dataset,file_name)
    ed_event_df = pd.read_csv(file_path)
    result_dir = 'D:\\Dataset\\standardized_data\\ECG\\mimic'
    result_file_name = '05_ed_event_cvd.csv'
    cvd_df = {}
    for key in ed_event_df.keys():
        cvd_df[key] = []
    # 2. 根据 icd code 筛选 CVD 患者
    all_diags = ed_event_df['icd_code'].values
    for idx, diag in enumerate(all_diags):
...
# TODO
def get_diag_from_note() -> None:
    """ match ecg & note -> get diagnose """
    # 0. prepare
    base_dir = 'D:\\Dataset\\original_data\\ECG'
    dataset_name= '20240801_mimics_iv'
    sub_dataset = 'mimic-iv-note'
    note_name = 'note\\radiology.csv'
    # 1. 读取 筛选的 ed ecg 数据列表, mimic-iv-note 全部诊断结果
    note_file_name = os.path.join(base_dir,dataset_name,sub_dataset,note_name)
    note_df = pd.read_csv(note_file_name)
    result_dir = 'D:\\Dataset\\standardized_data\\ECG\\mimic'
    ed_ecg_file_name = os.path.join(result_dir, '04_icd_ed.csv')
    ed_ecg_df = pd.read_csv(ed_ecg_file_name)

    # 2. 准备写入的文件
    result_file_name = '04_icd_ed_text.csv'
    ed_ecg_df['examination'] = np.empty(len(ed_ecg_df), dtype=object)
    ed_ecg_df['technique'] = np.empty(len(ed_ecg_df), dtype=object)
    ...
    eval_info.to_csv(result_file, index=False)
    result_file  = os.path.join(result_dir,result_file_name2)
    eval_info = pd.DataFrame(trouble_table)
    eval_info.to_csv(result_file, index=False)

def temp():
    """临时文件"""
    path_dir = 'D:\\Dataset\\original_data\\ECG\\20240801_mimics_iv\\mimic-iv-note\\note'
...
def main():
    ...
    get_cvd_from_ed_event()
    # # 6. 从 ed event 数据的 CVD 患者中，添加
    # identify_ed_icm_from_cvd()
